...
from mile.config.base import BaseConfig, BaseStrEnum
from mile.training.priors import PriorDist
...
class Sampler(BaseStrEnum):
    ...
    def get_kernel(self):
        """Get sampling kernel."""
        try:
            from mile.training.kernels import KERNELS
    ...
    def get_warmup_kernel(self):
        """Get warmup kernel."""
        try:
            from mile.training.kernels import WARMUP_KERNELS
        except ModuleNotFoundError:
            raise ModuleNotFoundError(
                'For Bayesian Inference, samplers are required: '
                'Please install "blackjax" module'
...
class SamplerConfig(BaseConfig):
    ...
    @property
    def kernel(self):
        """Returns the kernel: see mile.training.kernels for more details."""
